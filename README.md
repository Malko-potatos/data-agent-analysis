...
이 레포의 핵심 메시지는 간단합니다. 에이전트에게 바로 "분석해줘"라고 시키기 전에, 먼저 일이 어떤 데이터 분석 use case인지 보고, 그 결과를 어떤 자동화 use case로 이어갈지 정한 뒤, 데이터, 도구, 검증, 승인 경계를 붙여 실행합니다.

## What You Will Learn

- 사용자 요구사항을 업무 문제로 분해하는 법
- Codex use case와 분석/자동화 harness를 매칭하는 법
- 데이터 분석 use case와 자동화 use case를 조합하는 법
...
Natural request
-> Requirement card
-> Data use case matching
-> Automation use case matching
-> Combined Agent Workflow Harness
...
5. [docs/09-data-automation-playbook.md](docs/09-data-automation-playbook.md)에서 데이터 분석 use case와 자동화 use case를 조합합니다.
6. 비개발자 학습자는 [templates/data-analysis-agent-requirement-form.docx](templates/data-analysis-agent-requirement-form.docx)의 쉬운 요청 카드부터 작성합니다.
7. 에이전트와 함께 use case, harness, 검증 기준, 승인 경계를 보완합니다.
8. [examples/](examples/)의 예제로 수업 실습을 진행합니다.
...
수업이나 워크숍에서는 [templates/data-analysis-agent-requirement-form.docx](templates/data-analysis-agent-requirement-form.docx)를 먼저 나눠주고, 학생들이 쉬운 질문만 채우게 합니다. use case 라우팅은 "무슨 분석인가?"와 "분석 결과를 어떻게 반복/알림/승인 흐름으로 만들 것인가?"라는 두 질문으로 나눠 에이전트와 정리합니다.
