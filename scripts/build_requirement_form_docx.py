...
def build():
    ...
    add_label_detail_table(
        doc,
        [
            ("분석 유형", "데이터 분석 / 데이터 정리 / 피드백 분석 / 대시보드 / 기존 agent 점검 중 무엇에 가까운가?"),
            ("자동화 유형", "반복 리포트 / 대시보드 갱신 / 알림 queue / 회의 follow-up / 자동화 보류 중 무엇인가?"),
    ...
    add_response_box(
        doc,
        "에이전트에게 보낼 실행 요청 초안",
        "위 요청 카드를 바탕으로 필요한 자료를 먼저 확인하고, 데이터 분석 유형과 자동화 유형을 나눠 제안한 뒤, "
